  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto">
        ...
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 sm:mb-10 reveal reveal-up">
          <div>
        ...
        {/* Grid — 1 kolom mobile, 2 tablet, 4 desktop, lebih di layar besar */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {beritaList.map((b, i) => (
            <Link key={b.id} href={`/berita/${b.slug ?? b.id}`}
              className={`group flex flex-col overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl active:scale-[0.98] reveal reveal-up reveal-delay-${(i % 4) + 1}`}
              ...
              <div className="relative overflow-hidden" style={{ height: '180px', flexShrink: 0 }}>
                {b.gambar ? (
                  ...
                  <Image src={heroBg[i % heroBg.length]} alt={b.judul} fill
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                    className="object-cover transition-transform duration-500 group-hover:scale-105" />
                )}
                ...
                {b.kategori && (
                  <span className="absolute top-3 left-3 text-[9px] font-black px-2 py-0.5 rounded-md tracking-wider uppercase"
              ...
              <div className="flex flex-col flex-1 p-4 gap-2">
                ...
                <h3 className="text-sm font-semibold leading-snug line-clamp-3 flex-1 transition-colors group-hover:text-[#F5A623]"
                  style={{ color: 'rgba(255,255,255,0.85)' }}>
                  {b.judul}
                </h3>
                <div className="mt-2 pt-3 text-xs font-bold flex items-center gap-1 transition-all group-hover:gap-2"
