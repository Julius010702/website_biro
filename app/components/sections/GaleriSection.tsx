...
export default async function GaleriSection() {
  ...
  return (
    <section className="relative py-12 sm:py-16 lg:py-20 px-4 overflow-hidden" style={{ background: '#050D1A' }}>

      {/* ── Background ── */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <Image src="/images/hero/panorama-laut.jpeg" alt="" fill className="object-cover opacity-[0.10]" sizes="100vw" />
        ...
        <svg className="absolute inset-0 w-full h-full" style={{ opacity: 0.035 }}>
          <defs>
            <pattern id="gal-dots" x="0" y="0" width="28" height="28" patternUnits="userSpaceOnUse">
              <circle cx="2" cy="2" r="1.2" fill="white" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#gal-dots)" />
        </svg>
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-175 h-100 rounded-full pointer-events-none"
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        {/* ── Grid Desktop (sm+): featured spans 2x2 ── */}
        <div className="hidden sm:grid grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-3"
          ...
          {galeriList.map((g, i) => (
            <Link
              ...
              className={`group relative overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl active:scale-[0.98] ${
                i === 0 ? 'col-span-2 row-span-2' : ''
              }`}
              style={{ border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.45)', textDecoration: 'none' }}>
              <Image
                src={g.thumbnail ?? g.url} alt={g.judul} fill
              ...
              <div className="absolute inset-0 rounded-2xl border border-white/0 group-hover:border-white/12 transition-all duration-300" />
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl"
                style={{ boxShadow: 'inset 0 0 0 1px rgba(56,189,248,0.2)' }} />
            </Link>
          ))}
        </div>

        {/* ── Grid Mobile (< sm): 2 kolom, NO col-span ── */}
        <div className="grid sm:hidden grid-cols-2 gap-2" style={{ gridAutoRows: '130px' }}>
          {galeriList.map((g, i) => (
            <Link
              ...
              {/* Hover label */}
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-2.5"
                style={{ background: 'linear-gradient(to top, rgba(5,13,26,0.88), transparent)' }}>
                <span className="text-white text-[10px] font-semibold line-clamp-2 leading-snug">{g.judul}</span>
              </div>
              {i === 0 && (
        ...
        <div className="mt-6 sm:hidden text-center">
          <Link href="/informasi/galeri" className="inline-flex items-center gap-2 text-sm font-bold px-6 py-3 rounded-xl active:scale-95"
            style={{ background: '#38BDF8', color: '#050D1A' }}>
