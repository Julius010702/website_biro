  ...
  return (
    <>
      <div
        ...
        style={{ background: isHome ? 'transparent' : '#0A1929' }}
        id="site-header"
      >
        {/* TOP BAR */}
        <div
        ...
        >
          <div className="flex items-center flex-1 min-w-0 overflow-hidden">
            <div className="shrink-0 flex items-center gap-2 px-3 py-2"
              ...
              <span className="font-bold tracking-widest text-[10px] uppercase whitespace-nowrap" style={{ color: '#F5A623' }}>BIRO</span>
            </div>
        ...
        {/* GOLD LINE */}
        <div className="hidden md:block h-px w-full"
        ...
        <header
          className="w-full"
          style={{
            background: isHome ? 'rgba(10,25,41,0.55)' : '#0A1929',
            backdropFilter: isHome ? 'blur(16px)' : 'none',
            WebkitBackdropFilter: isHome ? 'blur(16px)' : 'none',
            boxShadow: isHome ? '0 1px 0 rgba(255,255,255,0.06)' : '0 2px 16px rgba(0,0,0,0.3)',
          }}
        >
          {/* max-w-screen-2xl — lebar di semua layar */}
          <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
            ...
            <Link href="/" className="flex items-center gap-3 shrink-0 group">
              <div className="relative w-11 h-11 shrink-0">
                <Image src="/images/logo-prov-ntt.png" alt="Logo Provinsi NTT" fill sizes="44px"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 right-0 bottom-0 overflow-y-auto overflow-x-hidden"
            style={{ top: `${headerHeight}px`, background: '#0A1929', borderTop: '1px solid rgba(245,166,35,0.2)' }}>
...
function DesktopNavItem({
  ...
  item: NavItem; active: boolean; onHover: (label: string) => void; onLeave: () => void
  activeSub: string | null; onSubHover: (label: string) => void; icon?: React.ReactNode
}) {
  const pathname   = usePathname()
  const closeTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  const isActive = item.href
    ? pathname === item.href
    : item.children?.some((c) => c.href ? pathname.startsWith(c.href) : c.children?.some((cc) => cc.href && pathname.startsWith(cc.href)))
  const handleEnter = () => { if (closeTimer.current) clearTimeout(closeTimer.current); if (item.children) onHover(item.label) }
    ...
    <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      {item.href && !item.children ? (
        ...
        <button className={cn('header-nav-link', (active || isActive) && 'active')}
          ...
          {icon}{item.label}
          {item.children && <ChevronDown className={cn('w-3.5 h-3.5 transition-transform duration-200', active && 'rotate-180')} />}
        </button>
      )}
      ...
      <div className="space-y-0.5">
        {items.map((item) => (
          <MobileNavItem key={item.label} item={item} expanded={expanded}
            onToggle={toggle} onClose={onClose} level={0}
            icon={navIcons[item.label]} />
        ))}
      </div>
