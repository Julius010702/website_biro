...
export default async function PPIDPage() {
  ...
  const infoCards = [
    { icon: <Shield        className="w-6 h-6" />, label: 'Dasar Hukum',    nilai: 'UU 14/2008',            sub: 'Keterbukaan Informasi Publik', color: '#0D47A1', bg: '#EFF6FF' },
    { icon: <FileText      className="w-6 h-6" />, label: 'Total Dokumen',  nilai: String(totalDokumen),    sub: 'Dokumen tersedia',             color: '#065F46', bg: '#ECFDF5' },
    { icon: <MessageSquare className="w-6 h-6" />, label: 'Permohonan',     nilai: String(totalPermohonan), sub: 'Permohonan masuk',             color: '#7C3AED', bg: '#F5F3FF' },
    { icon: <Users         className="w-6 h-6" />, label: 'Atasan PPID',    nilai: 'Kepala Biro',           sub: 'Biro Organisasi Setda NTT',    color: '#B45309', bg: '#FFFBEB' },
  ]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Hero card */}
      <div className="rounded-2xl p-6 sm:p-8 relative overflow-hidden reveal reveal-up"
        style={{ background: 'linear-gradient(135deg, #0A2342 0%, #0D47A1 60%, #1565C0 100%)' }}>
        <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.04]" xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="ppid-dot" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1.5" fill="white" /></pattern></defs>
          <rect width="100%" height="100%" fill="url(#ppid-dot)" />
        </svg>
        <div className="relative">
          <span className="inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.18em] uppercase px-3 py-1.5 rounded-full mb-4"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {infoCards.map((c, i) => (
          <div key={c.label} className={`rounded-2xl p-4 reveal reveal-up reveal-delay-${i + 1}`}
      ...
      {/* Tentang PPID */}
      <div className="rounded-2xl p-6 reveal reveal-left" style={{ background: 'white', border: '1px solid #DBEAFE' }}>
        ...
        <div className="text-sm text-slate-600 leading-relaxed space-y-3">
          <p>PPID (Pejabat Pengelola Informasi dan Dokumentasi) merupakan pejabat yang bertanggung jawab di bidang penyimpanan, pendokumentasian, penyediaan, dan/atau pelayanan informasi di badan publik sesuai amanat <strong>Undang-Undang Nomor 14 Tahun 2008</strong> tentang Keterbukaan Informasi Publik.</p>
          <p>Biro Organisasi Setda Provinsi NTT sebagai badan publik wajib menyediakan, memberikan, dan/atau menerbitkan informasi publik yang berada di bawah kewenangannya kepada pemohon informasi publik, selain informasi yang dikecualikan sesuai ketentuan peraturan perundang-undangan.</p>
          <p>Pelayanan informasi publik dilaksanakan pada hari dan jam kerja: <strong>Senin–Jumat, pukul 08.00–16.00 WITA</strong>.</p>
        </div>
      </div>

      {/* Maklumat singkat */}
      {maklumat && (
        <div className="rounded-2xl p-5 flex items-start gap-4 reveal reveal-up"
