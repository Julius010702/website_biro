...
export default async function BeritaPage({
  ...
  const [beritaList, total, kategoriRaw, beritaUtama] = await Promise.all([
    prisma.berita.findMany({
      where, orderBy: { createdAt: 'desc' }, skip, take: PER_PAGE,
      select: { id: true, judul: true, slug: true, ringkasan: true, gambar: true, kategori: true, createdAt: true, views: true },
    }),
    prisma.berita.count({ where }),
    prisma.berita.findMany({ where: { publish: true, kategori: { not: null } }, select: { kategori: true }, distinct: ['kategori'] }),
    !katParam && !queryParam && halaman === 1
      ? prisma.berita.findFirst({ where: { publish: true }, orderBy: { createdAt: 'desc' },
          select: { id: true, judul: true, slug: true, ringkasan: true, gambar: true, kategori: true, createdAt: true, views: true } })
  ...
  const displayList  = beritaUtama ? beritaList.filter((b) => b.id !== beritaUtama.id) : beritaList
    ...
    <main style={{ background: '#F4F7FD', minHeight: '100vh' }}>
      ...
      <section className="relative py-12 px-4 overflow-hidden"
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.04]" xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="b-dot" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1.5" fill="white" /></pattern></defs>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 reveal reveal-up">
            ...
            <form method="GET" action="/berita" className="flex items-center gap-2 w-full sm:w-72">
              ...
              <div className="relative flex-1">
                ...
                <input type="text" name="q" defaultValue={queryParam} placeholder="Cari berita..."
                  className="w-full text-sm pl-9 pr-3 py-2.5 rounded-xl outline-none"
                  style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white' }} />
              </div>
              <button type="submit" className="px-4 py-2.5 rounded-xl text-xs font-bold transition-all hover:scale-105"
                style={{ background: '#F5A623', color: '#0A2342' }}>Cari</button>
            </form>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {kategoriList.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-7 reveal reveal-up">
            <Link href={buildUrl({ kategori: '', halaman: 1 })}
              className="text-[11px] font-semibold px-3.5 py-1.5 rounded-full transition-all"
              style={{ background: !katParam ? '#0D47A1' : 'white', color: !katParam ? 'white' : '#1565C0', border: '1px solid #DBEAFE' }}>
              Semua
            </Link>
            {kategoriList.map((kat) => (
              <Link key={kat} href={buildUrl({ kategori: kat, halaman: 1 })}
                className="text-[11px] font-semibold px-3.5 py-1.5 rounded-full transition-all"
                style={{ background: katParam === kat ? '#0D47A1' : 'white', color: katParam === kat ? 'white' : '#1565C0', border: '1px solid #DBEAFE' }}>
                {kat}
              </Link>
            ))}
          </div>
        ...
        {(queryParam || katParam) && (
          <div className="flex items-center gap-2 mb-5">
            ...
            <Link href="/berita" className="text-[11px] font-semibold text-red-500 hover:text-red-700 underline">Reset</Link>
          </div>
        )}

        {/* Featured berita */}
        {beritaUtama && (
          <Link href={`/berita/${beritaUtama.slug}`}
            className="group flex flex-col sm:flex-row overflow-hidden rounded-2xl mb-8 hover:-translate-y-1 hover:shadow-xl transition-all reveal reveal-up"
            ...
            <div className="relative sm:w-2/5 overflow-hidden" style={{ minHeight: '220px' }}>
              {beritaUtama.gambar
                ? <BeritaImage src={beritaUtama.gambar} alt={beritaUtama.judul} fill sizes="(max-width: 640px) 100vw, 40vw"
                    className="object-cover transition-transform duration-700 group-hover:scale-105" />
                : <GambarPlaceholder size="lg" />
              }
              <span className="absolute top-3 left-3 text-[9px] font-black px-2.5 py-1 rounded-lg tracking-wider"
                style={{ background: '#F5A623', color: '#0A2342' }}>UTAMA</span>
              {beritaUtama.kategori && (
                <span className="absolute bottom-3 left-3 text-[10px] font-bold px-2 py-0.5 rounded"
                  style={{ background: 'rgba(255,255,255,0.92)', color: '#0D47A1' }}>{beritaUtama.kategori}</span>
              )}
            </div>
            <div className="flex flex-col justify-center flex-1 p-6 sm:p-8 gap-3">
              <h2 className="text-lg sm:text-xl font-bold leading-snug line-clamp-3 transition-colors group-hover:text-blue-700"
                style={{ color: '#0A2342' }}>{beritaUtama.judul}</h2>
              {beritaUtama.ringkasan && (
                <p className="text-sm text-slate-500 leading-relaxed line-clamp-3">{beritaUtama.ringkasan}</p>
              )}
              <div className="flex items-center gap-4 text-xs text-slate-400">
                <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{format(new Date(beritaUtama.createdAt), 'd MMMM yyyy', { locale: localeId })}</span>
                <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5" />{beritaUtama.views.toLocaleString()} kali dibaca</span>
              </div>
              <div className="mt-1 pt-3 flex items-center gap-1.5 text-sm font-semibold transition-all group-hover:gap-2.5"
        ...
        {displayList.length === 0 && !beritaUtama ? (
          <div className="rounded-2xl p-16 text-center" style={{ background: 'white', border: '1px solid #DBEAFE' }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {displayList.map((b, i) => (
              <Link key={b.id} href={`/berita/${b.slug}`}
                className={`group flex flex-col overflow-hidden rounded-2xl hover:-translate-y-1 hover:shadow-xl transition-all reveal reveal-up reveal-delay-${(i % 3) + 1}`}
                ...
                <div className="relative overflow-hidden shrink-0" style={{ height: '180px' }}>
                  {b.gambar
                    ? <BeritaImage src={b.gambar} alt={b.judul} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                        className="object-cover transition-transform duration-500 group-hover:scale-105" />
                    : <GambarPlaceholder size="md" />
                  }
                  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(10,35,66,0.65) 0%, transparent 55%)' }} />
                  {b.kategori && (
                    <span className="absolute bottom-3 left-3 text-[9px] font-bold px-2 py-0.5 rounded"
                      style={{ background: 'rgba(255,255,255,0.92)', color: '#0D47A1' }}>{b.kategori}</span>
                  )}
                </div>
                <div className="flex flex-col flex-1 p-4 gap-2">
                  <h3 className="text-sm font-bold leading-snug line-clamp-3 transition-colors group-hover:text-blue-700"
                    style={{ color: '#0A2342' }}>{b.judul}</h3>
                  {b.ringkasan && <p className="text-xs text-slate-400 leading-relaxed line-clamp-2">{b.ringkasan}</p>}
                  <div className="flex items-center justify-between mt-auto pt-2.5 text-[10px] text-slate-400"
                    style={{ borderTop: '1px solid #EEF3FC' }}>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{formatDistanceToNow(new Date(b.createdAt), { locale: localeId, addSuffix: true })}</span>
                    <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{b.views.toLocaleString()}</span>
                  </div>
                </div>
              </Link>
            ))}
              ...
              {Array.from({ length: totalHalaman }, (_, i) => i + 1)
                ...
                .map((p, i) => p === 'ellipsis'
                  ...
                  : <Link key={p} href={buildUrl({ halaman: p as number })}
                      ...
                      style={{ background: p === halaman ? '#0D47A1' : 'white', color: p === halaman ? 'white' : '#1565C0', border: '1px solid #DBEAFE' }}>
                      {p}
                    </Link>
                )}
            </div>
