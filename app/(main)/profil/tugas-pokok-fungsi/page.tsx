          ...
          <div className="grid lg:grid-cols-[1fr_300px] gap-8 items-start">
            <div className="flex flex-col gap-5">
              {items.length > 0 ? (
                  ...
                  <div key={item.id} className="rounded-2xl overflow-hidden"
                    ...
                    <div className="flex items-center gap-4 px-6 py-4"
                      ...
                      <div className="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-sm font-black"
                        style={{ background: 'linear-gradient(135deg, #0D47A1, #1565C0)', color: 'white', boxShadow: '0 3px 10px rgba(13,71,161,0.30)' }}>
                        {String(i + 1).padStart(2, '0')}
                      </div>
                      <h3 className="text-sm font-bold leading-snug" style={{ color: '#0A2342', fontFamily: 'var(--font-heading)' }}>
                        {item.judul}
                      </h3>
                    </div>
                      ...
                      <div className="flex flex-col gap-2">
                        {item.konten.split('\n').filter(Boolean).map((line, idx) => (
                          <div key={idx} className="flex items-start gap-2">
                            <span className="mt-1.5 w-1.5 h-1.5 rounded-full shrink-0" style={{ background: '#0D47A1' }} />
                            <p className="text-sm leading-relaxed" style={{ color: '#4B5563' }}>{line.trim()}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="rounded-2xl p-10 text-center" style={{ background: 'white', border: '1px solid #DBEAFE' }}>
            ...
            <ProfilSidebar activeHref="/profil/tugas-pokok-fungsi" title="Tugas Pokok & Fungsi" description="Uraian tugas dan fungsi Biro Organisasi Setda Provinsi NTT." />
          </div>
        </div>
      </section>
    </>
