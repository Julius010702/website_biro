...
function ImageUpload({ label, value, onChange }: { label: string; value: string; onChange: (u: string) => void }) {
  const [busy, setBusy] = useState(false)
  const [err, setErr]   = useState('')
  const ref = useState(() => typeof document !== 'undefined' ? document.createElement('input') : null)[0]

  useEffect(() => {
    if (!ref) return
    ref.type = 'file'; ref.accept = 'image/jpeg,image/png,image/webp'
    ref.onchange = async () => {
      const file = ref.files?.[0]; if (!file) return
      setErr(''); setBusy(true)
      try { onChange(await uploadImage(file)) }
      catch (e: unknown) { setErr(e instanceof Error ? e.message : 'Gagal') }
      finally { setBusy(false); ref.value = '' }
    }
  }, [ref, onChange])
    ...
    <FormField label={label}>
      <div className="flex flex-col gap-1.5">
        <div className="flex gap-2">
          <Input value={value} onChange={(e) => onChange(e.target.value)} placeholder="https://... atau upload" className="flex-1" />
          <button type="button" disabled={busy} onClick={() => ref?.click()}
...
export default function SekapurSirihPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/sekapur-sirih').then((r) => r.json()).then((d) => { if (d) setData(d) })
  }, [])
    ...
    start(async () => {
      try {
        await upsertSekapurSirih({
          ...
          foto: data.foto ?? undefined, jabatan: data.jabatan ?? undefined, nama: data.nama ?? undefined,
        })
      ...
      <AdminCard>
        ...
        <div className="p-5 grid sm:grid-cols-2 gap-4">
          <FormField label="Judul" required>
            <Input value={data.judul ?? ''} onChange={(e) => setData({ ...data, judul: e.target.value })} />
          </FormField>
          <FormField label="Nama Pejabat">
            <Input value={data.nama ?? ''} onChange={(e) => setData({ ...data, nama: e.target.value })} />
          </FormField>
          <FormField label="Jabatan">
            <Input value={data.jabatan ?? ''} onChange={(e) => setData({ ...data, jabatan: e.target.value })} />
          </FormField>
          <ImageUpload label="Foto Pejabat" value={data.foto ?? ''} onChange={(foto) => setData({ ...data, foto })} />
          <div className="sm:col-span-2">
            <FormField label="Konten / Sambutan" required>
              <Textarea rows={10} value={data.konten ?? ''} onChange={(e) => setData({ ...data, konten: e.target.value })} />
            </FormField>
          </div>
        </div>
