'use client'
// app/admin/ppid/informasi-publik/page.tsx
...
import {
  ...
  ExternalLink, Save, RefreshCw, Upload, CheckCircle,
} from 'lucide-react'
...
function UploadButton({
  ...
  endpoint: string  // nama endpoint di uploadthing core.ts
  value:    string
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Preview gambar */}
      {accept === 'image' && value && (
        <div className="relative w-full rounded-lg overflow-hidden border border-gray-100 bg-gray-50" style={{ height: 120 }}>
          <Image src={value} alt="preview" fill className="object-contain p-2" sizes="400px" />
          <button
            type="button"
            onClick={() => onChange('')}
      ...
      <div className="flex gap-2">
        <label className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg border-2 border-dashed cursor-pointer text-xs font-medium transition-colors ${
          disabled || uploading
          ...
          {uploading ? (
            ...
            <><CheckCircle className="w-4 h-4 text-emerald-500" /> Ganti {accept === 'image' ? 'Gambar' : 'Dokumen'}</>
          ) : (
            <><Upload className="w-4 h-4" /> Upload {accept === 'image' ? 'Gambar' : 'Dokumen'}</>
          )}
          <input
            type="file"
            className="hidden"
            accept={accept === 'image' ? 'image/*' : '.pdf'}
            onChange={handleFile}
        ...
        <input
          ...
          className="flex-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-xs focus:border-blue-500 focus:bg-white focus:outline-none"
          value={value}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <a href="/ppid/informasi-publik" target="_blank" rel="noopener noreferrer"
          ...
          <div className={`px-5 py-3.5 border-b border-gray-100 flex items-center justify-between ${editItem ? 'bg-amber-50' : 'bg-gray-50/70'}`}>
            <div className="flex items-center gap-2">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${editItem ? 'bg-amber-100' : 'bg-blue-700'}`}>
                {editItem ? <Pencil className="w-3 h-3 text-amber-700" /> : <Plus className="w-3 h-3 text-white" />}
              </div>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {(form.tipe === 'DOKUMEN' || form.tipe === 'GAMBAR_DOKUMEN') && (
                <div className="sm:col-span-2">
                  <UploadButton
                    label="Upload Dokumen (PDF, maks 16MB)"
                    accept="document"
                    endpoint="pdfUploader"
                    value={form.urlDokumen}
                    onChange={url => setForm(f => ({ ...f, urlDokumen: url }))}
                    disabled={saving}
                  />
                </div>
              )}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea rows={2}
                  className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:outline-none resize-none"
      ...
      <div className="flex flex-wrap gap-3">
        <input type="text" placeholder="Cari judul..."
          className="flex-1 min-w-48 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
        ...
        <button onClick={load}
          className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
            ...
            {filtered.map((item) => (
              <div key={item.id} className={`flex items-start gap-3 px-5 py-4 hover:bg-gray-50/80 transition-colors ${!item.aktif ? 'opacity-50' : ''}`}>
                ...
                {item.tipe !== 'DOKUMEN' && item.url ? (
                  <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 border border-gray-100 bg-gray-50 relative cursor-pointer"
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => toggleAktif(item)} title={item.aktif ? 'Sembunyikan' : 'Tampilkan'}
