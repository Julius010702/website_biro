...
async function main() {
  ...
  const hashedPassword = await bcrypt.hash('Admin@NTT2026', 12)
  await prisma.user.upsert({
    where: { email: 'biroorganisasisetdaprovinsintt@gmail.com' },
    update: {},
    create: {
      name: 'Super Admin',
  ...
  const settings = [
    ...
    { key: 'email', value: 'biroorganisasisetdaprovinsintt@gmail.com', label: 'Email' },
